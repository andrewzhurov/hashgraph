...
** TODO optionally, use transients more
* DONE fix inspector
...
* DONE perhaps layout viz horizontaly on wide displays
** DONE horizontal view
** DONE select view mode based on screen resolution
** DONE controls to switch between horizontal/vertical mode
* DONE animate received events
* DONE add explanation on first appearance of novel algorithm behaviour
* TODO playback controls
** DONE sync with scroll
** DONE playback one event
** DONE rewind back on scroll up
...
* DONE fix stuck round
That's perhaps due to dirty mem lookup.
* TODO better peek viz
** TODO pretty scale
** DONE fix dimming of votes
* CANCELLED instead of opacity use lightened color for stake map
Tried with garden utils. Ended up being costly.
* DONE more animation
** DONE witness
** DONE round
** DONE stake map
...
* DONE hardly reachable members may issue it's stale event, which will make refs look not pretty
[[~/notes/org-roam/store/sha256/cba97c7bda1f19a7835ebf4c531f784986ce1befb32492c3d3e4638238490e89]]
* CANCELLED maybe lookup from mem as goog.object will be faster than (transient (hash-map))
...
*** DONE Perhaps it's better to perform that ad-hoc, when user wishes to inspect.
Thus algo can stay lean and fast.
...
Still reproduces? Happens on mobile.
** DONE >>| does nothing
** TODO |<< transitions are slowpoke
** DONE arrow on home may not be an intuitive action to scroll
Now it scrolls once.
...
*** DONE somewhat fixed by dropping selected from tutors stuff when that tutor hides
** TODO Problem: dragging scrollbar results in jittery scroll
Observed on somewhat old Chromium. On new version it's okay.
...
** TODO fix gossip to gossip to one random member
*** although [[https://hyp.is/r2lF-vo0Ee6NZY9fP-Azgg/www.swirlds.com/downloads/SWIRLDS-TR-2016-01.pdf][here in paper]] member send his event to two members
**** how it works specified [[https://hyp.is/8NzAmPo0Ee683dNgpWB0cQ/www.swirlds.com/downloads/SWIRLDS-TR-2016-01.pdf][here]]
*** TODO what if member's unreachable?
**** perhaps to broadcast to another
*** TODO what if member receives it but does not include in their DAG?
**** we'll have diminishing amount of gossip, eventually reaching 0
**** perhaps wrap event in message with address
*** TODO what if member cheats by accepting an event that's been sent to someone else?
**** E.g., Bob sends an event to Charlie, but Charlie selects some event he doesn't know from Bob's event, but not the Bob's event.
**** perhaps wrap event in message with address
* objectives
...
** links to tutors, ala ?tutor=round_concluded
Can be useful for pointing to Hashgraph's behaviour.
** switch to garden color data model
*** add color viz to inspector
** DONE make algorithm viz mesmerizing
...
tape of a1 [a0 b0 a1]
#+end_src
subjective order of events, as they've been received, idxs are stable
** embrace columnar model
#+begin_src
             a0 b0 a1
creator      0  1  0
self-parent  _  _  0
...
txes     [tx1 tx2]
#+end_src
** perhaps vector as self-parent chain
*** where event is a vector
*** where vec element is a ref to tape
*** how performance compares with maps?
** rewrite it in Rust
** multi-key creator
** inspectable messages
** inspectable read receipts
** shown messages as f(last-played-event)
** +C to appear conditionally as f(clipboard-text)
*** only when there's not yet such outbound contact & it's not my
** contact names
*** as unique part of did, as in members
